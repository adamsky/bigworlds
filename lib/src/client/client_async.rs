...
use crate::msg::{
    DataPullRequest, DataPullResponse, DataTransferRequest, DataTransferResponse,
    ExportSnapshotRequest, ExportSnapshotResponse, Message, NativeQueryRequest,
    NativeQueryResponse, PingRequest, PullRequestData, QueryRequest, RegisterClientRequest,
    RegisterClientResponse, ScheduledDataTransferRequest, StatusRequest, StatusResponse,
    StepAdvanceRequest, TransferResponseData, TypedSimDataPack,
};
use crate::portal::{
    Address as PortalAddress, CompositeAddress, Encoding, NodeAsync, NodeConfig, SocketEventType,
    Transport,
};
use crate::{
    error::Error, portal, portal::NodeEvent, query, string, time::Instant, Address, Result, Var,
};

use super::{ClientConfig, CompressionPolicy};

/// Represents an asynchronous interface for communicating with the server.
pub struct ClientAsync {
    /// Configuration struct
    config: ClientConfig,
    /// Connection to server
    pub connection: NodeAsync,
...
impl ClientAsync {
    ...
    pub fn new_with_config(config: ClientConfig) -> Result<Self> {
        ...
        let connection = NodeAsync::new_with_config(None, transport, socket_config)?;
        let client = Self {
            config,
            connection,
            connected: false,
    ...
    pub async fn connect(&mut self, greeter_addr: &str, password: Option<String>) -> Result<()> {
        ...
        self.connection = NodeAsync::new_with_config(None, transport, socket_config)?;

        // self.connection.manual_poll()?;

        self.connection
            .connect(greeter_composite.address.clone())
            .await?;
        ...
        info!("sent client registration request");
        ...
        info!("got response from server: {:?}", resp);
        //
    ...
    pub async fn disconnect(&mut self) -> Result<()> {
        ...
        self.connection.disconnect(None).await?;
        Ok(())
    }
    ...
    pub fn manual_poll(&mut self) -> Result<()> {
        ...
        if let Some(hb_interval) = self.config.heartbeat_interval {
            if self.time_since_heartbeat >= hb_interval {
                self.connection
                    .send_event(NodeEvent::new(SocketEventType::Heartbeat), None)?;
    ...
    pub async fn server_status(&mut self) -> Result<StatusResponse> {
        self.connection.send_payload(
            StatusRequest {
                format: "".to_string(),
            },
        ...
        let (_, msg) = self.connection.recv_msg().await?;
        let resp: StatusResponse = msg.unpack_payload(self.connection.encoding())?;
        Ok(resp)
    }

    pub async fn server_step_request(&mut self, steps: u32) -> Result<Message> {
        self.connection.send_payload(
            StepAdvanceRequest {
                step_count: steps,
                wait: true,
        ...
        let (_, resp) = self.connection.recv_msg().await?;
        Ok(resp)
    }

    // data querying
    pub async fn get_var(&mut self, addr: &str) -> Result<Var> {
        // self.connection.send_payload(
        //     DataTransferRequest {
        ...
        self.connection.send_payload(
            NativeQueryRequest {
                query: query::Query {
                    ...
                    filters: vec![query::Filter::Name(vec![string::new_truncate(
                        "earth_climate",
                    )])],
                    mappings: vec![query::Map::Components(vec![string::new_truncate("temp")])],
                },
            },
            None,
        )?;
    ...
    pub async fn get_vars(&mut self) -> Result<TransferResponseData> {
        self.connection.send_payload(
            DataTransferRequest {
    ...
    // mutation
    pub async fn set_vars(&mut self, vars: Vec<(Address, Var)>) -> Result<()> {
        self.connection.send_payload(
            DataPullRequest {
        ...
        if resp.error.is_empty() {
            ...
            Err(Error::ErrorResponse(resp.error))
        }
    }

    pub async fn snapshot_request(&mut self, name: String, save_to_disk: bool) -> Result<Vec<u8>> {
        ...
        println!("sent");
        let (_, v) = self.connection.recv_msg().await?;
